...
export const AVAILABLE_AGENTS: Agent[] = [
	{
		...
		name: "Deep Agent",
		description:
			"General purpose AI agent capable of handling tasks of varying complexity",
		deploymentUrl:
			import.meta.env.VITE_DEPLOYMENT_URL || "http://127.0.0.1:2024",
	},
	{
		id: "research-agent",
		name: "Research Agent",
	...
	},
	{
		...
		name: "Coding Agent",
		description:
			"Expert agent focused on software development and coding tasks",
		deploymentUrl:
			import.meta.env.VITE_DEPLOYMENT_URL || "http://127.0.0.1:2024",
	},
	{
...
export async function fetchAgentsFromAPI(
	...
	try {
		...
		if (!deployment.deploymentUrl || !deployment.workspaceId) {
			console.warn(
				"Missing deployment configuration, falling back to hardcoded agents",
			);
			return AVAILABLE_AGENTS;
		}
		...
		if (currentAgent?.id) {
			...
			if (storedParams) {
				try {
					...
					console.error(
						"Failed to parse agent parameters from localStorage:",
						error,
					);
				}
			}
